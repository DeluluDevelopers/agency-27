      ...
      <div className="w-full max-w-7xl mx-auto px-2 xs:px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-2xl xs:text-4xl md:text-5xl font-bold text-white mb-4 heading-accent">
            Success <span className="bg-gradient-to-r from-blue-400 to-violet-400 bg-clip-text text-transparent">Stories</span>
          </h2>
          <p className="text-base xs:text-xl text-gray-300 max-w-xs xs:max-w-3xl mx-auto">
            Real results from real clients. See how we've transformed businesses with our rapid development approach.
          </p>
        </motion.div>
        ...
        >
          {/* Case Study Image */}
          <motion.div whileHover={{ scale: 1.02 }} className="relative group w-full max-w-md mx-auto lg:max-w-full">
            <div
            ...
            />
            <img
              src={caseStudies[activeCase].image || "/placeholder.svg?height=320&width=480"}
          ...
          <div className="space-y-4 xs:space-y-6">
            <div>
              ...
              <h3 className="text-xl xs:text-3xl font-bold text-white mb-2 xs:mb-4">{caseStudies[activeCase].title}</h3>
              <p className="text-sm xs:text-lg text-gray-300 leading-relaxed">{caseStudies[activeCase].description}</p>
            </div>
            ...
            <div className="flex flex-row flex-wrap justify-center items-stretch gap-2 xs:gap-4 w-full">
              {caseStudies[activeCase].metrics.map((metric, index) => (
                ...
                >
                  ...
                  <div className="font-bold text-white text-base xs:text-lg">{metric.value}</div>
                  <div className="text-xs xs:text-sm text-gray-400">{metric.label}</div>
                </motion.div>
              ))}
            </div>
            ...
            {/* Tech Stack */}
            <div>
              <div className="text-xs xs:text-sm font-semibold text-gray-300 mb-2 xs:mb-3">Technologies Used:</div>

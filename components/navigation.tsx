        ...
        {isMobileMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 bottom-0 left-0 z-50 flex flex-col items-center justify-center bg-gray-900/95 md:hidden outline-none"
              role="dialog"
              aria-modal="true"
              tabIndex={-1}
            >
              ...
              <nav className="flex flex-col gap-8 w-full items-center mt-12 px-4">
                {navItems.map((item) => (
                  <a
                    key={item.name}
                    href={item.href}
                    className="text-2xl font-semibold text-gray-200 hover:text-blue-400 transition-colors px-8 py-4 rounded-lg w-full text-center focus:outline-none focus:ring-2 focus:ring-blue-400"
                    style={{ minWidth: 120 }}
                    onClick={(e) => {
                      setIsMobileMenuOpen(false);
